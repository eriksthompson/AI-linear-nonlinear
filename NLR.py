...
def calc_jacobian(X,p):
    ...
    for x in range(X.shape[0]):
        ...
        J[x, 0] = X[x, 0] ** p[1,0]
        J[x, 1] = p[0,0] * (X[x,0] ** p[1,0]) * np.log(X[x,0])
        J[x, 2] = X[x,0]
        J[x, 3] = 1
...
def nonlinear_regression_gn(X, Y, initialP):
    ...
    for iteration in range(GAUSS_NEWTON_ITERATIONS): #PLEASE do not change this line
        ...
        J = calc_jacobian(X, p)
        m1 = np.linalg.inv(np.matmul(J.transpose(), J))
        m2 = np.matmul(J.transpose(), deltaY)
        deltaP = np.matmul(m1,m2)
        p = p + deltaP
    ### Your job ends here ###
    return p
...
def nonlinear_regression_gd(X, Y, initialP):
    ...
    # PLEASE use LEARNING_RATE variable defined above
        ...
        J = calc_jacobian(X, p)
        deltaY = Y - model_function(X, p)
        gradientE = -2 * np.matmul(J.transpose(), deltaY)
        #update parameters
        p = p - (LEARNING_RATE * gradientE)
    ### Your job ends here ###
    return p
